...
export async function GET(request) {
  ...
  function drawHeader(p) {
    ...
    let ty = boxY + boxH - 16;
    p.drawText(`Collection de ${safe(user.username)}`, { x: boxX + 10, y: ty, size: 11, font: fontBold, color: HEADER_TEXT });
    ty -= 14;
    p.drawText(`${cards.length} carte(s) archivee(s)`, { x: boxX + 10, y: ty, size: 8, font, color: HEADER_MUTED });
  }
  ...
  if (cards.length === 0) {
    page.drawText("Aucune carte archivee pour l'instant.", { x: marginX, y, size: 10, font: fontItalic, color: MUTED });
  }
  ...
  return new Response(pdfBytes, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="collection-${safe(user.username)}.pdf"`,
    },
  });
}

...
/**
 * Propose un scan pour un effet/prisme qui n'existe pas encore pour ce numéro (carte
 * sœur de référence passée en paramètre) — même redimensionnement que submitCardScan,
 * envoyé à un point d'entrée séparé qui créera une nouvelle carte une fois validé.
 */
/**
 * Propose un scan pour un effet/prisme qui n'existe pas encore pour ce numéro (carte
 * sœur de référence passée en paramètre) — même redimensionnement que submitCardScan,
 * envoyé à un point d'entrée séparé qui créera une nouvelle carte une fois validé.
 */
export async function submitCardVariant(referenceCardId, file, rarete) {
  const resizedBlob = await resizeInBrowser(file, {});
  const fd = new FormData();
  fd.append("file", resizedBlob, "scan.jpg");
  fd.append("rarete", rarete);

  const res = await fetch(`/api/cards/${referenceCardId}/variant-submissions`, { method: "POST", body: fd });
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data.error || "Échec de l'envoi du scan.");
  }
...
/**
 * Propose une carte non encore répertoriée pour une collection dont le total est encore
 * inconnu (voir le slot "Carte recherchée" permanent sur la page collection).
 */
export async function submitNewCardForCollection(collectionId, file, { numero, rarete, personnagePrincipalId }) {
  const resizedBlob = await resizeInBrowser(file, {});
  const fd = new FormData();
  fd.append("file", resizedBlob, "scan.jpg");
  fd.append("numero", numero);
  fd.append("rarete", rarete || "Commune");
  if (personnagePrincipalId) fd.append("personnagePrincipalId", personnagePrincipalId);

  const res = await fetch(`/api/collections/${collectionId}/new-card-submissions`, { method: "POST", body: fd });
  const data = await res.json();
  if (!res.ok) {
...
/**
 * Ajoute une carte à une proposition de collection en cours de construction (voir
 * /compte/proposer-collection/[id]) — même redimensionnement que les autres uploads.
 */
export async function submitCollectionSubmissionCard(submissionId, file, { numero, rarete, personnagePrincipalId }) {
  const resizedBlob = await resizeInBrowser(file, {});
  const fd = new FormData();
  fd.append("file", resizedBlob, "scan.jpg");
  fd.append("numero", numero);
  ...
  const res = await fetch(`/api/collection-submissions/${submissionId}/cards`, { method: "POST", body: fd });
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data.error || "Échec de l'envoi.");
  }
